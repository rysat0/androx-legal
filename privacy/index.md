...
株式会社StepAI（以下「当社」といいます。）は、当社が提供する男性向けヘルスケアアプリ「Androx」（以下「本サービス」といいます。）における利用者の個人情報、健康・フィットネス関連情報およびその他の利用者情報の取扱いについて、以下のとおりプライバシーポリシー（以下「本ポリシー」といいます。）を定めます。

本サービスは、医療上の診断、治療、投薬指導、または専門的な医療助言を提供するものではありません。本サービス内のスコア、インサイト、記事、レポート、通知その他の情報は、一般的なウェルネスおよび生活習慣管理を目的とした情報であり、血中テストステロン値その他のホルモン値を測定、診断、推定または保証するものではありません。健康状態、症状、治療方針、投薬、検査値等に関する判断については、医師その他の医療専門家にご相談ください。

## 第1条 適用範囲

本ポリシーは、本サービスの利用に関連して、当社が利用者から取得する情報の取扱いに適用されます。

## 第2条 当社が取得する情報

当社は、本サービスの提供に必要な範囲で、以下の情報を取得または処理する場合があります。

### 1. アカウント情報

本サービスでは、Apple Sign-InおよびSupabase Authを利用してアカウント認証を行います。当社がサーバー上で処理する情報は、以下に限定されます。

1. メールアドレス
2. Apple Sign-Inにより付与されるユーザー識別子
3. Supabase Authにより作成される認証用ユーザーID
4. アカウント削除処理に必要な一時的なApple authorization code

表示名は、メールアドレスのローカル部をもとに端末内で初期化される場合がありますが、当社サーバーには送信されません。

### 2. プロフィールおよびオンボーディング情報

本サービスでは、以下のプロフィール情報およびオンボーディング回答を、利用者の端末内にのみ保存する場合があります。

1. 主目的（natural optimizer、low T curious、on TRT、fertility conscious 等の選択情報）
2. 主目標
3. 週あたりのトレーニング回数
4. 食事傾向
5. 身長
...
7. 単位設定
8. 目標スコア
9. その他、利用者が本サービス内で入力する設定情報

生年月日、年齢、目標体重は、機能上必要な範囲で一時的に利用される場合がありますが、ディスクには保存されず、当社サーバーにも送信されません。

### 3. Apple Health / HealthKit由来の健康・フィットネス情報

利用者が明示的に許可した場合、本サービスはApple HealthまたはHealthKitから以下の健康・フィットネス関連情報を読み取る場合があります。
...
睡眠分析データは、合計睡眠時間、深睡眠、REM睡眠、入眠時刻等に分解して、本サービス内のスコアおよび表示に利用される場合があります。

本サービスは、HealthKitへの書き込みを行いません。HealthKit由来データはすべて利用者の端末内で処理・保存され、当社サーバー、Supabase、外部クラウド、広告事業者、分析事業者、AIサービス等には送信されません。

### 4. 手入力ログ

本サービスのRecordタブ等において、利用者が任意で入力した場合、以下の情報を端末内に保存する場合があります。
...
これらの手入力ログはすべて利用者の端末内にのみ保存され、当社サーバーには送信されません。性的活動に関するログはセンシティブな情報であるため、当社はこれを広告、マーケティング、分析、販売、第三者提供の目的で利用しません。

### 5. 検査値・ラボデータ

本サービスは、現時点において、Total Testosterone、Free Testosterone、SHBG、Estradiolその他の血液検査値またはラボデータを取得、保存、送信しません。本サービス内のLearn記事等に検査値やホルモンに関する教育的情報が含まれる場合がありますが、利用者の検査値を入力・保存する機能はありません。

### 6. 主観的な症状ログ

本サービスは、現時点において、気分、リビドー、集中力、回復感その他の主観的な症状ログを取得、保存、送信しません。

### 7. 技術情報

本サービスでは、広告SDK、解析SDK、クラッシュ解析SDK、アトリビューションSDK、AI SDKは使用していません。当社がサーバー上で処理する技術情報は、Supabase Authによるアカウント認証およびアカウント削除処理に必要な範囲に限定されます。
...
1. Apple Health / HealthKit由来データ
2. 手入力ログ
3. 性的活動に関するログ
4. スコア
5. 週次レポート
6. プロフィールおよびオンボーディング情報
7. アプリ内設定
8. ローカル通知に関する設定

これらの情報は、当社サーバー、Supabase、外部クラウド、広告事業者、分析事業者、AIサービス等には送信されません。本サービスは、現時点において、iCloudまたはCloudKitによる同期を行いません。

### 2. サーバー上で処理される情報

当社は、アカウント認証のため、Supabase Authを利用します。Supabaseに送信・保存される情報は、Apple Sign-Inを通じて取得されるメールアドレスおよび認証用ユーザー識別子に限定されます。アカウント削除時には、削除処理のため、Apple authorization codeがSupabaseの削除用機能に一時的に送信される場合があります。

### 3. データ書き出し機能

利用者が本サービス内のデータ書き出し機能を自ら実行した場合、端末内の一部ログがJSON形式で生成され、共有シート等を通じて端末外に共有される場合があります。書き出される項目は、日付、ならびにトレーニング実施・寒冷暴露・日光曝露・サプリメント摂取・禁酒・良い睡眠の各記録、および（入力がある場合に限り）睡眠時間・体重・カロリー・歩数に限定されます。性的活動に関するログ、ならびにHRV・体脂肪率・睡眠ステージ・安静時心拍数等の詳細な生体情報は、書き出しに含まれません。

この共有は利用者自身の明示的な操作によってのみ行われ、当社は当該データを当社サーバーに送信または保存しません。

## 第4条 利用目的

当社は、取得または処理した情報を、以下の目的で利用します。

1. 本サービスの提供、運営、維持のため
2. Apple Sign-InおよびSupabase Authによるアカウント認証のため
3. アカウント削除、問い合わせ対応その他のユーザーサポートのため
4. 睡眠、運動、回復、体組成、生活習慣に基づくスコアを端末内で計算するため
5. 利用者にパーソナライズされたアクション、通知、リマインダー、週次レポートを端末内で提供するため
6. 本サービスの安全性、品質、利便性の維持・改善のため
7. 不正利用、セキュリティ事故、規約違反等の防止および調査のため
8. 法令遵守、紛争対応、監査、権利保護のため
9. 上記に付随または関連する目的のため

当社は、HealthKit由来データ、健康・フィットネス関連情報、手入力ログ、性的活動に関するログを、広告、マーケティング、ターゲティング広告、利用者追跡、データブローカーへの提供、販売の目的で利用しません。

## 第5条 Apple Health / HealthKitデータの取扱い

当社は、利用者の明示的な許可がある場合に限り、Apple HealthまたはHealthKitから健康・フィットネス関連情報を読み取ります。当社は、Apple HealthまたはHealthKit由来データを以下の目的に限定して利用します。

1. 本サービス内でのスコア表示
2. 睡眠、運動、回復、体組成、生活習慣に関するインサイトの提供
3. 利用者に対するアクション提案
4. 週次レポートおよびリマインダーの提供
5. 利用者自身による健康・ウェルネス管理の補助

当社は、Apple HealthまたはHealthKit由来データを、当社サーバー、Supabase、外部クラウド、広告事業者、分析事業者、AIサービス、データブローカー等に送信しません。当社は、Apple HealthまたはHealthKit由来データを、広告、マーケティング、利用者追跡、または利用ベースのデータマイニング目的で利用しません。

## 第6条 AI機能の不使用

本サービスは、現時点において、OpenAIその他の大規模言語モデル、外部AI API、AI SDK、AIによる個人データ処理機能を使用していません。本サービスのスコア計算、アクション提案、週次レポート等は、利用者の端末内で実行される決定論的なロジックに基づいて生成されます。当社は、利用者の個人情報、HealthKit由来データ、健康・フィットネス関連情報、手入力ログ、性的活動に関するログを、AIサービスに送信しません。

## 第7条 広告・解析・トラッキングの不使用

本サービスは、現時点において、以下を使用していません。
...
当社は、利用者の個人情報、HealthKit由来データ、健康・フィットネス関連情報、手入力ログ、性的活動に関するログを販売しません。

## 第8条 第三者サービス・委託先

当社は、本サービスのアカウント認証およびアカウント削除機能を提供するため、Supabaseを利用します。Supabaseは、Apple Sign-Inを通じて取得されるメールアドレスおよび認証用ユーザー識別子を、当社の委託先として処理します。当社は、HealthKit由来データ、手入力ログ、性的活動に関するログ、スコア、週次レポート、プロフィール情報、オンボーディング情報をSupabaseに送信しません。

当社は、現時点において、Firebase、Google Analytics、AdMob、Meta Pixel、Mixpanel、Amplitude、PostHog、Sentry、AppsFlyer、RevenueCat、Stripe、OneSignal、OpenAIその他の外部AIサービスを使用していません。
...
4. 公的機関から法令に基づく要請を受けた場合
5. 合併、会社分割、事業譲渡その他の事業承継に伴って提供される場合
6. 利用目的の達成に必要な範囲で業務委託先に提供する場合

当社は、利用者のHealthKit由来データ、健康・フィットネス関連情報、手入力ログ、性的活動に関するログを、広告事業者、分析事業者、AIサービス、データブローカーに販売または共有しません。

## 第10条 Cookie等の利用

本サービスのiOSアプリは、広告目的またはクロスアプリトラッキング目的でCookie、モバイル広告ID、IDFAその他の識別子を使用しません。当社が本サービスに関連するウェブサイトを運営する場合、当該ウェブサイトにおいてCookie等を利用することがあります。その場合、当社は、Cookie等の利用目的、送信先、オプトアウト方法等について、利用者が確認できる方法で表示します。

## 第11条 データの保存期間およびアカウント削除

利用者の端末内に保存される情報は、利用者がアプリ内で削除する、アプリを削除する、または端末上のデータを削除するまで、端末内に保存されます。

Supabase Auth上で処理されるメールアドレスおよび認証用ユーザー識別子は、アカウントの維持に必要な期間保存されます。利用者がアプリ内の「アカウント削除」機能を実行した場合、当社はSupabase Auth上の当該アカウント情報を速やかに削除し、あわせて当該端末内に保存されているローカルデータ（記録、週次レポート等）も削除のうえ、サインアウトを行います。なお、法令上保存が必要な情報、紛争対応・不正利用防止・監査等のために必要な情報については、合理的な期間内に削除または匿名化します。

## 第12条 安全管理措置

当社は、取得または処理する情報の漏えい、滅失、毀損、不正アクセス等を防止するため、必要かつ適切な安全管理措置を講じます。当社が講じる安全管理措置には、以下が含まれます。
...
本サービスにおけるHealthKit由来データおよび手入力ログは、利用者の端末内に保存され、当社サーバーには送信されません。

## 第13条 データ漏えい等への対応

当社は、個人情報の漏えい、滅失、毀損、不正アクセス等が発生し、利用者の権利利益を害するおそれがある場合、適用法令に従い、利用者、監督機関その他必要な関係者への通知または報告を行います。米国の利用者に関して、本サービスがFTC Health Breach Notification Ruleその他適用法令の対象となる場合、当社は当該法令に従い必要な通知を行います。

## 第14条 利用者の権利

利用者は、適用法令に従い、当社に対して以下を請求できる場合があります。
...
利用者がこれらの請求を希望する場合は、本ポリシー末尾のお問い合わせ先までご連絡ください。当社は、本人確認を行ったうえで、適用法令に従い対応します。

## 第15条 米国居住者向け補足

米国の一部州の居住者は、適用されるプライバシー法に基づき、個人情報の開示、削除、訂正、販売または共有のオプトアウト、センシティブ個人情報の利用制限等の追加の権利を有する場合があります。カリフォルニア州のCCPAでは、健康、性生活、性的指向に関する情報等がセンシティブ個人情報に含まれると説明されています。

当社は、利用者の個人情報を販売しません。また、HealthKit由来データ、健康・フィットネス関連情報、手入力ログ、性的活動に関するログを、広告事業者、分析事業者、AIサービス、データブローカーに販売または共有しません。

## 第16条 未成年者の利用

本サービスは、18歳以上の利用者を対象としています。18歳未満の方は、本サービスを利用しないでください。当社が18歳未満の利用者から個人情報を取得したことを認識した場合、当社は、適用法令に従い、当該情報の削除その他必要な措置を講じます。

## 第17条 国際的なデータ処理

本サービスは日本法人である株式会社StepAIにより提供されます。Supabase Auth等の第三者サービスを利用する場合、メールアドレスおよび認証用ユーザー識別子等のアカウント情報が、日本国外を含むサーバーまたはサービスプロバイダーにおいて処理される場合があります。当社は、適用法令に従い、必要かつ適切な保護措置を講じます。

## 第18条 本ポリシーの変更

当社は、法令改正、サービス内容の変更、運用上の必要性等に応じて、本ポリシーを変更する場合があります。重要な変更を行う場合、当社は、本サービス内での通知、ウェブサイトへの掲載、その他適切な方法により利用者に通知します。

## 第19条 お問い合わせ

本ポリシーまたは当社の個人情報の取扱いに関するお問い合わせは、以下までご連絡ください。

- 個人情報取扱事業者：株式会社StepAI（英語表記：StepAI Co., Ltd.）
